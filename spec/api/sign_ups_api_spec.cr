# #Clerk Backend API
#
##The Clerk REST Backend API, meant to be accessed by backend servers.  ### Versions  When the API changes in a way that isn't compatible with older versions, a new version is released. Each version is identified by its release date, e.g. `2025-03-12`. For more information, please see [Clerk API Versions](https://clerk.com/docs/versioning/available-versions).  Please see https://clerk.com/docs for more information.
#
#The version of the OpenAPI document: 2025-03-12
#Contact: support@clerk.com
#Generated by: https://openapi-generator.tech
#Generator version: 7.12.0
#

require "../spec_helper"
require "json"
require "time"

# Unit tests for Clerk::SignUpsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe "SignUpsApi" do
  describe "test an instance of SignUpsApi" do
    it "should create an instance of SignUpsApi" do
      api_instance = Clerk::SignUpsApi.new
      # TODO expect(api_instance).to be_instance_of(Clerk::SignUpsApi)
    end
  end

  # unit tests for get_sign_up
  # Retrieve a sign-up by ID
  # Retrieve the details of the sign-up with the given ID
  # @param id The ID of the sign-up to retrieve
  # @param [Hash] opts the optional parameters
  # @return [SignUp]
  describe "get_sign_up test" do
    it "should work" do
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  # unit tests for update_sign_up
  # Update a sign-up
  # Update the sign-up with the given ID
  # @param id The ID of the sign-up to update
  # @param [Hash] opts the optional parameters
  # @option opts [UpdateSignUpRequest] :update_sign_up_request 
  # @return [SignUp]
  describe "update_sign_up test" do
    it "should work" do
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

end
